...
# Create original style adaptive icon (transparent background)
size = 1024
img = Image.new('RGBA', (size, size), (0, 0, 0, 0))  # Transparent background
draw = ImageDraw.Draw(img)

# Draw bar chart (white bars) - same pattern as original
bar_width = 80
bar_spacing = 40
bars_heights = [240, 390, 310, 470, 330]  # Scaled up for adaptive icon
...
# Draw AI text (white) - larger for adaptive icon
text = 'AI'
text_x = size // 2 - 120
text_y = size // 2 - 50
...
print('Original style adaptive icon created successfully!')
